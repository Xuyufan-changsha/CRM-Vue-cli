<template>
	...
	<el-menu :default-active="$route.path" class="mar-l el-menu-vertical-demo el-col el-col-3" light router>
      <el-menu-item-group :title="this.$t('message.userSystem')">
        <div v-for="(item,index) in blog_item" v-if="!item.hidden">
  ...
  export default {
    data() {
      return {
        ...
        blog_item: [
          {
            title: this.$t('message.userInfo'),
            path: '/index/user',
            class: 'fa-user'
          }, {
            title: this.$t('message.admin'),
            path: '/index/article',
            class: 'fa-newspaper-o'
          },
          {
            title: this.$t('message.reimbursement'),
